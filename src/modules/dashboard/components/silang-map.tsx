...
function createPinIcon(
  ...
  const shapeMap: Record<DamageLevel, () => string> = {
    ...
    `,
    /* DESTROYED — spiky burst / explosion */
    DESTROYED: () => `
  ...
  return (
    ...
    >
      ...
      {barangays.map((b) => {
        ...
        const damageColor = assessment ? damageLevelToColor(assessment.damageLevel) : '#4a5568'
        const damageLabel = assessment ? (DAMAGE_LABEL[assessment.damageLevel] ?? assessment.damageLevel) : 'No Data'

        const icon = assessment
          ? createPinIcon(damageColor, assessment.damageLevel, assessment.damagePercent)
          : NO_DATA_ICON
